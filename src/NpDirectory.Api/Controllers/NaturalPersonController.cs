...
public class NaturalPersonController : ControllerBaseWrapper
{
    private readonly INaturalPersonsService _naturalPersonsService;

    public NaturalPersonController(INaturalPersonsService naturalPersonsService, ILogger<NaturalPersonController> logger)
        : base(logger)
    {
    ...
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public Task<IActionResult> GenerateReport([FromQuery] GenerateReportRequest request)
        => ExecuteAsync(() => _naturalPersonsService.GenerateReportAsync(request));
}