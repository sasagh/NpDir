...
public class RelationService : IRelationService
{
    ...
    {
        var firstPerson = await _uof.NaturalPersonsRepository.GetFirstByFilterAsync(np => np.Id == request.FromId);
        if(firstPerson == null)
            throw new NotFoundException(_localizer["Error.NaturalPerson.NotFound"] + $" id: {request.FromId}");
        
        var secondPerson = await _uof.NaturalPersonsRepository.GetFirstByFilterAsync(np => np.Id == request.ToId);
        if(secondPerson == null)
            throw new NotFoundException(_localizer["Error.NaturalPerson.NotFound"] + $" id: {request.ToId}");
        
        var relationExists = await _uof.RelationsesRepository.GetFirstByFilterAsync(r =>
    ...
    {
        var relation = await _uof.RelationsesRepository.GetFirstByFilterAsync(r =>
