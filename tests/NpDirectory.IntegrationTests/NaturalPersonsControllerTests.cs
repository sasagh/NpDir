...
public class NaturalPersonControllerTests : IAsyncLifetime
{
    private readonly WebApplicationFactory<Program> _factory;
    private readonly HttpClient _client;
    private readonly string _dbName;

    public NaturalPersonControllerTests()
    {
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/naturalperson/1");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/naturalperson/999");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/naturalperson/fast-search?searchTerm=John");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/naturalperson/search?firstName=John");
    ...
    public async Task CreateNaturalPerson_WithValidData_ReturnsOk()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("/api/naturalperson", request);

        // Assert
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        // Verify creation
        var getResponse = await _client.GetAsync("/api/naturalperson/3");
    ...
    public async Task UpdateNaturalPerson_WhenExists_ReturnsOk()
    {
        ...
        // Act
        var response = await _client.PutAsJsonAsync("/api/naturalperson/1", request);

        // Assert
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        // Verify update
        var getResponse = await _client.GetAsync("/api/naturalperson/1");
        var content = await getResponse.Content.ReadFromJsonAsync<GetNaturalPersonInfoResponse>();
    ...
    {
        // Act
        var response = await _client.DeleteAsync("/api/naturalperson/2");

        // Assert
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        // Verify deletion
        var getResponse = await _client.GetAsync("/api/naturalperson/2");
    ...
    public async Task GenerateReport_WithValidRequest_ReturnsOk()
    {
        // Arrange
        // Act
        var response = await _client.GetAsync($"/api/naturalperson/report");
    ...
    public async Task UpdateNaturalPersonImage_WithValidImage_ReturnsOk()
    {
        ...
        // Act
        var response = await _client.PutAsync("/api/naturalperson/1/image", content);
