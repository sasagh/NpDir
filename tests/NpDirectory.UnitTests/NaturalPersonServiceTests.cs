...
public class NaturalPersonsServiceTests
{
    ...
    public async Task CreateNaturalPersonAsync_WhenPersonNotExists_CreatesNewPerson()
    {
        ...
        _uowMock.Setup(x => x.NaturalPersonsRepository.GetFirstByFilterAsync(It.IsAny<Expression<Func<NaturalPerson, bool>>>()))
            .ReturnsAsync((NaturalPerson)null);
        _uowMock.Setup(x => x.PhoneNumbersRepository.GetFirstByFilterAsync(It.IsAny<Expression<Func<PhoneNumber, bool>>>()))
    ...
    public async Task UpdateNaturalPersonAsync_WhenPersonExists_UpdatesPerson()
    {
        ...
        _uowMock.Setup(x => x.NaturalPersonsRepository.GetSingleByIdAsync(id))
    ...
    [Fact]
    public async Task GenerateReportAsync_ReturnsReport()
    {
        // Arrange
        var request = new GenerateReportRequest { Page = 0, PageSize = 10 };
        var relations = new List<Relation>
        {
            new()
            {
                NaturalPerson = new NaturalPerson { Id = 1, FirstName = "John" },
                RelatedPerson = new NaturalPerson { Id = 2, FirstName = "Jane" },
                Type = RelationType.Acquaintance
            }
        };

        _uowMock.Setup(x => x.RelationsesRepository.GetManyByFilterAsync(
                It.IsAny<Expression<Func<Relation, bool>>>(),
                request.Page,
                request.PageSize,
                It.IsAny<Expression<Func<Relation, object>>[]>()))
        ...
        Assert.Contains(result.Persons, p => p.Id == 1);
        Assert.Contains(result.Persons, p => p.Id == 2);
    }
}